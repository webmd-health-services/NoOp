...
Describe ('NoOp.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_NoOp' -Exists
    }
}

Describe ('NoOp.command verbs') {
...
Describe ('NoOp.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'NoOp' -CommandType Function,Cmdlet,Filter))
        {
